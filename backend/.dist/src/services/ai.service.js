...
class AIService {
    constructor() {
        ...
        if (env_1.default.gemini.apiKey) {
            this.genAI = new generative_ai_1.GoogleGenerativeAI(env_1.default.gemini.apiKey);
    ...
    async detectDuplicates(title, description, location) {
        ...
        const existingGrievances = await database_1.default.grievance.findMany({
            where: whereClause,
            select: {
                id: true,
                title: true,
        ...
        for (const grievance of existingGrievances) {
            if (grievance.embedding && grievance.embedding.length > 0) {
                ...
                const embeddingArray = Array.isArray(grievance.embedding)
                    ? grievance.embedding
                    : JSON.parse(grievance.embedding);
    ...
    // ============================================================
    async generateAutoResponse(title, description, category, grievanceId, userName, language = 'en') {
        const expectedDays = RESOLUTION_TIMES[category] || 15;
        const departmentName = INDIA_DEPARTMENTS[category] || 'District Collector Office';
        if (!this.model) {
...
Respond with ONLY a JSON object (no markdown):
{
  ...
  "trackingInfo": "Brief tracking instructions"
    ...
    getTemplateResponse(category, grievanceId, userName, expectedDays, departmentName, language) {
        const templates = {
            en: {
    ...
    getDefaultAcknowledgment(grievanceId, userName, category, language) {
        if (language === 'hi') {
...
Analyze and respond with ONLY a JSON object (no markdown, no explanation):
{
  ...
  "reasoning": "brief explanation"
            ...
            if (jsonMatch) {
                ...
                const department = INDIA_DEPARTMENTS[category] || INDIA_DEPARTMENTS['Other'];
                // Look up departmentId from database
                const deptRecord = await database_1.default.department.findFirst({
                    where: { category },
                    select: { id: true },
                });
                return {
    ...
    async keywordCategorize(title, description) {
    ...
    // ============================================================
    async fullAnalysis(title, description, userName, imagePaths = [], location) {
        ...
        const autoResponse = await this.generateAutoResponse(title, description, categorization.category, 'PENDING', // Will be replaced with actual ID after creation
        userName, titleTranslation.detectedLanguage);
        ...
        const aiAnalysisHash = crypto_1.default
            .createHash('sha256')
